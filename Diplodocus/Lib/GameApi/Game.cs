...
namespace Diplodocus.Lib.GameApi
{
    public static unsafe class Game
    {
        ...
        public static void Initialize(GameGui gameGui, SigScanner sigScanner)
        {
            try
            {
                uiModule = gameGui.GetUIModule();
                ProcessChatBox = Marshal.GetDelegateForFunctionPointer<ProcessChatBoxDelegate>(sigScanner.ScanText("48 89 5C 24 ?? 57 48 83 EC 20 48 8B FA 48 8B D9 45 84 C9"));
            }
            catch { PrintError("Failed to load /qexec"); }

            try { walkingBoolPtr = sigScanner.GetStaticAddressFromSig("88 83 33 05 00 00"); } // also found at g_PlayerMoveController+523
            ...
            try
            {
                ...
                try
                {
                    OpenAbandonDuty = Marshal.GetDelegateForFunctionPointer<OpenAbandonDutyDelegate>(sigScanner.ScanText("E8 ?? ?? ?? ?? EB 90 48 8B CB"));
                ...
                try
                {
                    UseItem = Marshal.GetDelegateForFunctionPointer<UseItemDelegate>(sigScanner.ScanText("E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 41 B0 01 BA 13 00 00 00"));
            ...
            try { actionCommandRequestTypePtr = sigScanner.ScanText("02 00 00 00 45 8B C5 89"); } // Located 1 function deep in Client__UI__Shell__ShellCommandAction_ExecuteCommand
            catch { PrintError("Failed to load /qac"); }

            try
            {
                keyStates = (int*)sigScanner.GetStaticAddressFromSig("4C 8D 05 ?? ?? ?? ?? 44 8B 0D"); // 4C 8D 05 ?? ?? ?? ?? 44 8B 1D
                keyStateIndexArray = (byte*)(sigScanner.Module.BaseAddress + *(int*)(sigScanner.ScanModule("0F B6 94 33 ?? ?? ?? ?? 84 D2") + 4));
            }
            catch { PrintError("Failed to load /sendkey!"); }

            try { ActionManager = sigScanner.GetStaticAddressFromSig("41 0F B7 57 04"); } // g_ActionManager
